...
psi_0 = np.zeros(sp_basis.Ns, dtype = np.float64) # allocate space for state
...
if dis_flag ==1:
    ...
    for j in range(L):
        params_dict_quench["z"+str(j)] = W/2*np.random.uniform(-1,1) # create quench random fields list
...
np.savetxt(nomefile, np.real(np.c_[t_tab, Losch, return_rate, i_t]), fmt = '%.9f')
